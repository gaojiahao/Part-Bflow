 ...
 * 业务组可以是业务单元或业务模块
 */
EditorUi.prototype.saveRoletaskBizGroup = function (forceDialog) {
	// if (!forceDialog && this.editor.filename != null) {
	// 	this.saveBizGroup(this.editor.getOrCreateFilename());
	// }
	// else {
		var dlg = new BizGroupnameDialog(this, this.editor.getOrCreateFilename(), mxResources.get('save'), mxUtils.bind(this, function (name) {
			this.saveBizGroup(name);
		}), '名称', mxUtils.bind(this, function (name) {
			if (name != null && name.length > 0) {
				return true;
			}
 ...
 * 将当前图形保存在给定的名称
 */
EditorUi.prototype.saveBizGroup = function (name) {
	if (name != null) {
		...
		var xml = this.editor.getGraphXml(),
			apps = xml.getElementsByTagName('UserObject'),
			appIds = [];

		

		var sxml = mxUtils.getXml(xml);

		for(var i =0 ;i<apps.length;i++){
			appIds.push(apps[i].getAttribute('listId'));
		}
		var data = {
			name: name,
			businessGroupXml: sxml,
			type: urlParams.type,
			listIds: appIds.toString()
		};

		var saveUrl = !urlParams.id ?'saveBusinessGroup':'updateBusinessGroup';

		if(urlParams.id){
			data.id = urlParams.id;
		} 
		$._rfd_http('/H_roleplay-si/trans/' + saveUrl, 'POST', JSON.stringify(data),true,function (res) {
			window.top.changeToViewing(res.type, res.id);
		});
